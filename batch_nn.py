...
def trainop(train_x,train_y,Ws,yita):
    ...
    for i in range(layer_num,0,-1):
        ...
        w_delta=np.dot(layers[i-1].T,init_de)
        init_de = np.dot(init_de,Ws[i - 1][:-1,:].T )
        Ws[i-1]+=w_delta

    return Ws,loss


def train(train_x,train_y,units,yita,batchsize=5,epoch=20000):
    '''
    :param train_x:数据
    :param train_y:标签
    :param units:层数
    ...
    step=int(len(train_x)/batchsize)
    losses = []
    steps = []
    ...
    for i in range(epoch):
        ...
        for j in range(step):
            this_batch_x=train_x[j*batchsize:(j+1)*batchsize]
            this_batch_y=train_y[j*batchsize:(j+1)*batchsize]
            W_li,loss=trainop(this_batch_x,this_batch_y,W_li,yita)
...
if __name__=='__main__':
    ...
    units=[5]
    train(train_x,train_y,units,0.001,batchsize=5)
